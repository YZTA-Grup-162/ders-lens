...
export const CameraDetector: React.FC<CameraDetectorProps> = ({
  ...
  const captureAndAnalyze = useCallback(async () => {
    ...
    try {
      const response = await fetch('http://localhost:8003/analyze/frame', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          image: imageData,  // Send full data URL instead of just base64
          timestamp: Date.now(),
      ...
      if (response.ok) {
        ...
        // Handle the AI service response format: {success: true, data: {...}}
        const aiData = result.success ? result.data : result;
        
        // Extract face detection status from the AI response
  ...
  const startAnalysis = useCallback(() => {
    ...
    intervalRef.current = setInterval(captureAndAnalyze, 2000); // Analyze every 2 seconds
  }, [captureAndAnalyze]);
